...
import { getUserAppointmentsPage, cancelAppointment, getAppointmentById } from '@/api/appointment';
...
const UserAppointments: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await getUserAppointmentsPage(params);
  ...
  const handleViewDetail = async (record: Appointment) => {
    try {
      const response = await getAppointmentById(record.id);
      if (response.data && response.data.code === 200) {
    ...
    <div style={{ padding: '24px' }}>
      <Card>
        <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Title level={4} style={{ margin: 0 }}>我的预约</Title>
